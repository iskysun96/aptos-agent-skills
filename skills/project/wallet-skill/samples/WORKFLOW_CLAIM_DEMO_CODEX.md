# Workflow — End-to-End Claim Demo (Codex)
...
Codex can invoke skills explicitly:

- In CLI/IDE, run `/skills` or type `$` to mention a skill, then paste the prompt for that step.
...
- ensure frontend reads module address from the repo’s env/config convention (e.g., VITE_MODULE_ADDRESS)
