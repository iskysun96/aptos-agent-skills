...
## Priority 1: Enhance write-contracts Skill with Data Storage Decision Tree ✅ COMPLETE
...
**Root cause:** Developers don't know which storage/collection type to use
...
- [x] Section added to write-contracts skill
- [x] File size remains compressed (241 lines, well under 300 limit)
- [x] Decision tree covers all 6 storage types (exceeded 3 patterns goal)
- [x] Examples are inline and brief
- [ ] Tested with staking contract scenario (pending)
- [ ] No design mistakes in subsequent testing (pending)

### Implementation Summary

**What was delivered:**

1. **Comprehensive storage framework** - Expanded beyond initial Table vs move_to to cover all 6 storage types:
   - Vector, SmartVector, Table, TableWithLength, OrderedMap, BigOrderedMap
   - Deprecated SmartTable properly documented

2. **Progressive disclosure structure:**
   - Minimal addition to SKILL.md (~47 lines)
   - 4 detailed reference files in `skills/write-contracts/references/`:
     - `storage-decision-tree.md` - Decision framework with 2-3 questions
     - `storage-types.md` - Detailed type comparison with code examples
     - `storage-patterns.md` - 9 use-case patterns with smart defaults
     - `storage-gas-optimization.md` - Gas optimization strategies

3. **Smart defaults table** - 9 common patterns:
   - User registry → Table
   - Staking records → Table
   - Leaderboard → BigOrderedMap
   - Transaction log → SmartVector/Vector
   - Whitelist → Vector
   - Voting records → TableWithLength
   - Config mappings → OrderedMap
   - DAO proposals → BigOrderedMap
   - Asset collections → Vector/SmartVector

4. **Enhanced YAML frontmatter** - Added storage-related triggers

5. **File organization:**
   - Total SKILL.md size: 241 lines (194 → 241, +47 lines)
   - Storage section: Concise with references to detailed files
   - References added to existing references section

**Impact:**
- Prevents costly design mistakes (15k+ token refactors)
- Provides immediate decision framework when storage is mentioned
- Covers all storage types comprehensively
- Maintains compressed, scannable format
